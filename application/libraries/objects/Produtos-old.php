...
class Produtos extends db\Querys  {
    ...
    */
     ...
     * @access PUBLIC
     */
    public function getImageDefaultDiretory() {
    ...
    public function getProdutosByCart( $produtos = array() ) {
        ...
        if( count( $produtos ) > 0 ) {
            ...
            foreach ($produtos as $key => $value ) {
                ...
                $result[$i]['valu']  =  $data[0]->prod_id;

                $result[$i]['prod']  =  $data[0]->prod_nome;

                $result[$i]['refe']  =  $data[0]->prod_referencia;

                $result[$i]['codi']  =  $data[0]->prod_codigo;

                $result[$i]['qtd']   =  $value['qtd'];

                $result[$i]['valo']  =  number_format( $data[0]->prod_valor, 2, ',', '.' );
